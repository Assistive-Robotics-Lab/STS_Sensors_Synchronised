...
def seat_callback(msg):
    ...
    seat_arr = np.append([seat_arr], [msg.header.stamp, msg.data])
...
def balance_callback(msg):  
    ...
    balance_arr = np.append([balance_arr], [msg.header.stamp, msg.front_left, msg.front_right, msg.rear_left, msg.rear_right])
...
def xsens_callback(msg): ##Assuming that this is the fastest sensor
    ...
    #python array test
    xsens_arr = np.append([xsens_arr], [msg.header.stamp, msg.data])
...
def read_sensors():
 ...
 'field.poses22.orientation.x', 'field.poses22.orientation.y', 'field.poses22.orientation.z', 'field.poses22.orientation.w']
    ...
    #creating emmpty dataframes to fill with 1000 rows of data
    xsens_arr = []
    balance_arr = []
    seat_arr = []
    ...
    while not rospy.is_shutdown():
        ...
        if balance_check == 1:
            if len(seat_arr) > 100:
                ...
                seat_arr = pd.DataFrame(seat_arr)
                balance_arr = pd.DataFrame(balance_arr)
                xsens_arr = pd.DataFrame(xsens_arr)
